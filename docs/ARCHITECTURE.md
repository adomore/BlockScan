...
> 精确统计(`#[cfg(test)]` 前为生产代码,之后为内联单测;截至提交 `7546584`)。CI 每次推送自动复核测试/clippy/覆盖率三道门禁。
...
- **测试 640 个**(535 单元 + 105 集成),`cargo clippy --all-targets` 零告警;**全工作区行覆盖 97.76%**(区域 97.27%、函数 98.86%)——由 CI 的 `coverage` job **每次推送实测**、门禁设在 97%,非手工维护。此处只记百分比:精确行数每次提交都会微动,以最近一次 job 日志为准。
...
| 模块 | 内容 | 依据 |
|---|---|---|
| **绑定图后续(Phase 23+)** | 把 Phase 22 的绑定图用到 reentrancy(任意外部方法调用面 + 跨文件继承状态)、access-control(消除名字启发式)、delegatecall 局部 alias 回溯;跨合约同名 shadow 彻底消除 | AUDIT_DESIGN Phase 22「后续」 |
...
| [AUDIT_DESIGN.md](AUDIT_DESIGN.md) | 安全审计引擎 Phase 8–22 ✅(标准化/深度规则/SARIF/抑制/SCWE-EthTrust/AST 精化/数据流/reentrancy/access-control/weak-randomness/ecrecover/arbitrary-delegatecall/transfer-send/收窄-cast/**绑定图 scope-aware 类型解析**) | ✅ |
